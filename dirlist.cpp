    ...
    if (dir != 0) {
        ...
        while ((dirent = readdir(dir)) != 0) {
            ...
            switch (fileType(name)) {
                case CPP:
                    if (ac <= 0) {
                        insert_source(hasMainFunction, name, cpp, cpp_main,
                                      cpp_other);
                    }
                    break;
                ...
                case C:
                    if (ac <= 0) {
                        insert_source(hasMainFunction, name, c, c_main,
                                      c_other);
                    }
                    break;
                ...
                case ASSEMBLY:
                    if (ac <= 0) {
                        insert_source(hasMainLabel, name, ass, ass_main,
                                      ass_other);
                    }
                    break;
...
DirList::FileType DirList::fileType(const string &name) const {
    ...
    if (name.find('#') != string::npos) {
        cerr << "Warning: ignoring file whose name contains '#': " << name
             << endl;
        return OTHER;
    }
    return ft;
...
bool DirList::matchString(ifstream &in, char &ch, const string &s) {
    ...
    return (in.eof() &&
            n == in.gcount());  // return true if all characters were matched
                                // and the stream reached the end-of-file
}
