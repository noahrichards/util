...
void Write::write_header(bool noDebugFlag, bool useGCC, string header_file) {
    ...
    cout << '\n';
    if (!noDebugFlag) {
        cout << "########## Default flags (redefine these with a header.mak "
                "file if desired)\n";
        if (useGCC) {
...
void Write::write_main_targets(DirList &dirlist, string programs_file,
                               string custom_targets_file) {
    string local_libs(!dirlist.archive.empty() ? " $(LOCAL_LIBS)" : "");
    ifstream in;
    ...
    if (in) {  // read lines of "programs.mak"
        ...
        cout << "#\n"
             ...
             << "#\n\n"
             << "all:";
        for (vector<string>::iterator lit = lines.begin(); lit != lines.end();
             ++lit) {
            string line = *lit;
            string::iterator colon = find(line.begin(), line.end(), ':');
            if (colon != line.end()) {
        ...
        cpplinkdone:
            ...
            if (cpplink) {
                cout << '\t' << "$(CXX) $(CXXFLAGS) -o "
                     ...
                     << string(colon + 1, line.end()) << local_libs
                     << " $(CCLIBFLAGS)"
                     << "\n\n";
            } else {
                cout << '\t' << "$(CC) $(CFLAGS) -o "
        ...
        cout << "\n\n";
        write_main_target_list(dirlist.cpp_main, "$(CXX) $(CXXFLAGS)",
                               local_libs + " $(CCLIBFLAGS)");
        write_main_target_list(dirlist.c_main, "$(CC) $(CFLAGS)",
                               local_libs + " $(CLIBFLAGS)");
        write_main_target_list(dirlist.ass_main, "$(CC) $(CFLAGS)",
                               local_libs + " $(CLIBFLAGS)");
    }
    ...
    if (custom_targets) {
        ...
        copy(istreambuf_iterator<char>(custom_targets),
             istreambuf_iterator<char>(), ostreambuf_iterator<char>(cout));
        cout << '\n';  // in case no end-of-line at end of file
        cout << "########## End targets from " << custom_targets_file << "\n\n";
    }
}

void Write::write_main_target_list(const set<string> &list, string compile,
                                   string local_libs) {
    for (set<string>::const_iterator it = list.begin(); it != list.end();
         ++it) {
        string basename(DirList::basename(*it));

        cout << bin_dir << basename << bin_suffix << ":\t" << basename
             << ".o $(OBJFILES)" << '\n';
        cout << "\t@mkdir -p " << bin_dir << "\n";

        cout << '\t' << compile << " -o " << bin_dir << basename << bin_suffix
...
void Write::write_dependency_list(const set<string> &list, DirList &dirlist) {
    for (set<string>::const_iterator nameit = list.begin();
         nameit != list.end(); ++nameit) {
        string name(*nameit);
        set<string> includes(dirlist.includeFiles(name));
        set<string> dependencies;

        for (set<string>::iterator it = includes.begin(); it != includes.end();
             ++it) {
            if (false) {
...
void Write::write_trailer(DirList &dirlist, bool useGCC) {
    ...
    if (products.empty()) {
        ...
        for (set<string>::iterator it = products.begin(); it != products.end();
             ++it) {
            cout << *it << ' ';
        }
    }
